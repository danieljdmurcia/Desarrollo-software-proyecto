    ...
    def login(self, db: Session, email: str, password: str):
        ...
        if usuario.bloqueado_hasta:
            ...
            if ahora < bloqueado_hasta:
                ...
                minutos  = segundos // 60
                segs     = segundos % 60
                raise ValueError(f"Cuenta bloqueada. Intenta de nuevo en {minutos}m {segs}s")
            else:
        ...
        if not verificar_password(password, usuario.password_hash):
            ...
            if usuario.intentos_fallidos >= MAX_INTENTOS:
                ...
                usuario.intentos_fallidos = 0
                db.commit()
                raise ValueError(f"Demasiados intentos fallidos. Cuenta bloqueada por {BLOQUEO_MINUTOS} minutos")
            db.commit()
            raise ValueError(f"Email o contraseña incorrectos. Te quedan {restantes} intento(s)")
